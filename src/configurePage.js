...
<head>
    ...
    <style>
        ...
        body { margin: 0; padding: 0; font-family: 'Segoe UI', Roboto, sans-serif; background: linear-gradient(135deg, #0f0c29 0%, #1a237e 50%, #28143e 100%); color: #fff; display: flex; justify-content: center; align-items: center; min-height: 100vh; }
        .container { background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(10px); border-radius: 20px; padding: 50px 40px; max-width: 500px; width: 90%; margin: 50px 0; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.5); border: 1px solid rgba(255, 255, 255, 0.1); text-align: center; }
        .logo { width: 280px; margin-bottom: 20px; border-radius: 25px; box-shadow: 0 4px 15px rgba(138, 43, 226, 0.6); }
        ...
        .tooltip { position: relative; display: inline-block; margin-left: 8px; cursor: pointer; color: #a8c0ff; font-weight: bold; font-size: 0.75em; border: 1px solid #a8c0ff; border-radius: 50%; width: 14px; height: 14px; line-height: 12px; text-align: center; }
        .tooltip .tooltiptext { visibility: hidden; width: 200px; background-color: rgba(0,0,0,0.9); color: #fff; text-align: center; border-radius: 6px; padding: 8px; position: absolute; z-index: 1; bottom: 125%; left: 50%; margin-left: -100px; opacity: 0; transition: opacity 0.3s; font-size: 0.9em; font-weight: normal; }
        .tooltip:hover .tooltiptext { visibility: visible; opacity: 1; }
        .input-wrapper { display: flex; gap: 10px; margin-bottom: 5px; }
        input[type="text"] { flex: 1; padding: 12px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); background: rgba(0, 0, 0, 0.4); color: #fff; font-size: 1em; transition: all 0.3s ease; }
        input[type="text"]:focus { outline: none; border-color: #8a2be2; box-shadow: 0 0 0 3px rgba(138, 43, 226, 0.3); }
        .test-btn { padding: 12px 15px; border-radius: 8px; border: none; background: #8a2be2; color: #fff; cursor: pointer; font-weight: 500; transition: all 0.3s ease; min-width: 70px; }
        ...
        .link { display: inline-block; font-size: 0.8em; color: #a8c0ff; text-decoration: none; opacity: 0.8; margin-top: 5px; }
        ...
        .install-btn { width: 100%; padding: 15px; border-radius: 10px; border: none; background: linear-gradient(45deg, #8a2be2, #6a0dad); color: white; font-size: 1.3em; font-weight: 700; cursor: pointer; margin-top: 20px; transition: transform 0.2s, box-shadow 0.2s; box-shadow: 0 4px 15px rgba(138, 43, 226, 0.4); }
        ...
        .source-badge { background: rgba(255,255,255,0.08); padding: 10px 15px; border-radius: 10px; font-size: 0.8em; font-weight: 600; color: #fff; display: flex; align-items: center; border: 1px solid rgba(138, 43, 226, 0.3); box-shadow: 0 2px 8px rgba(0,0,0,0.2); }
...
<body>
    <div class="container">
        ...
        <div class="form-group">
            <label>SubDL API Key <span class="tooltip">?<span class="tooltiptext">Great alternative source for movies and series.</span></span></label>
        ...
        <div class="form-group">
            <label>SubSource API Key <span class="tooltip">?<span class="tooltiptext">Community-driven subtitle source.</span></span></label>
            <div class="input-wrapper">
        ...
        <div class="form-group">
            <label>Wyzie API Key <span class="tooltip">?<span class="tooltiptext">Free API, excellent for anime.</span></span></label>
            <div class="input-wrapper">
    ...
    <script>
        ...
        function renderTags() { const container = document.getElementById('selectedLangs'); container.innerHTML = ''; selectedLangs.forEach(lang => { const tag = document.createElement('div'); tag.className = 'tag'; tag.innerHTML = langNames[lang] + ' <span class="remove" onclick="removeLang(\\''+lang+'\\')">&times;</span>'; container.appendChild(tag); }); }
        function removeLang(lang) { selectedLangs = selectedLangs.filter(l => l !== lang); renderTags(); }
        function filterLangs() { const search = document.getElementById('langSearch').value.toLowerCase(); const list = document.getElementById('langList'); list.innerHTML = ''; const filtered = availableLangs.filter(l => langNames[l].toLowerCase().includes(search) && !selectedLangs.includes(l)); if (filtered.length > 0) { list.style.display = 'block'; filtered.forEach(l => { const item = document.createElement('div'); item.className = 'lang-item'; item.innerText = langNames[l] + ' (' + l + ')'; item.onclick = () => { if (selectedLangs.length >= 3) { alert('You can select a maximum of 3 languages.'); return; } selectedLangs.push(l); document.getElementById('langSearch').value = ''; list.style.display = 'none'; renderTags(); }; list.appendChild(item); }); } else { list.style.display = 'none'; } }
        async function testAPI(type) { const inputId = type + 'ApiKey'; const btn = event.target; const key = document.getElementById(inputId).value.trim(); if (!key) return; btn.innerText = '...'; btn.className = 'test-btn'; try { const res = await fetch('/test-api?type=' + type + '&key=' + key); const data = await res.json(); if (data.valid) { btn.innerText = 'Valid!'; btn.className = 'test-btn valid'; } else { const errText = data.error ? data.error.substring(0, 12) : 'Invalid!'; btn.innerText = errText; btn.title = data.error || 'Invalid API Key'; btn.className = 'test-btn invalid'; } } catch (e) { btn.innerText = 'Error!'; btn.className = 'test-btn invalid'; } }
